...
class CamVid(object):
    ...
    def generators(self):
        ...
        x_generator = CropImageDataGenerator(**self.data_gen_args)
        # create the segmentation data generator as a One-Hot tensor
        y_generator = CropNumpyDataGenerator(**self.data_gen_args)
        # the dictionaries to hold generators by key value
        generators = {}
        # iterate over the subsets in the generators
        for subset in ['training', 'validation']:
            x = x_generator.flow_from_directory(X_DIR, subset=subset, **self.flow_args)
            y = y_generator.flow_from_directory(self.y_dir, subset=subset, **self.flow_args)
